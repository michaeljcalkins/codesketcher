module.exports = function (grunt) {
  ...
  grunt.registerTask('release-mac-distributable', 'Releases the distributable for Mac', function () {
    ...
    if (buildOptions.platform === 'all' || buildOptions.platform === 'darwin') {
      ...
      var upload = function (key, secret) {
        var params = {
          localDir: 'releases/mac/hello-world-setup',
          s3Params: {
        ...
        uploader.on('error', function (err) {
          grunt.log.error('Unable to upload mac distributable to s3 ', err.stack)
          done(false)
        })
        ...
        uploader.on('end', function () {
          grunt.log.ok('Done uploading mac distributable to s3')
          done()
        })
      }
